...
class _Products_PageState extends State<Products_Page> {
              ...
              children: [
                ...
                InkWell(
                  onTap: () {
                    ...
                    // originSelect = [];
                    // manufacturerSelect = [];
                    if(manufacturer.isNotEmpty){   manufacturerSelect.forEach((element) {
                      manufacturer.firstWhere((e) => e.id==element,orElse: () => Manufacturer()).check=true;
                    });}
                    if(origin.isNotEmpty){

                      originSelect.forEach((element) {
                        origin.firstWhere((e) => e.id==element,orElse: () => Origin()).check=true;
                      });
                    }

                    if(categories.isNotEmpty){
                      partSelect.forEach((element) {
                    categories.firstWhere((e) => e.id==element,orElse: () => Categories_item()).Check=true;
                      });
                    }
                    showDialog(
                        context: context,
                        ...
                        .then((partSelect) {
                          if(partSelect==null){
                            ...
                            API(context)
                                .get(widget.Istryers
                                ? 'search/ahmed/category/parts${"${!widget.Istryers
                                ? ''
                                : "?width=${widthID ??
                                ''}&height=${heightID ??
                                ''}&size=${sizeID ??
                                ''}"}"}'+"${Filtration.contains('&', 0)
                                ? '${Filtration}'
                                : '&${Filtration}'}"
                                : url +
                                "${Filtration.contains('&', 0)
                  ...
                  onTap: () {
                    showDialog(
                      ...
                      API(context)
                          ...
                          :
                          '${url +
                              "${Filtration==null?'':Filtration.contains('&', 0)
                                  ? '${Filtration}'
                                  : '&${Filtration}'}"}&sort_type=${sort ??
                              'ASC&ordered_by=price'}')
                          .then((value) {
                        if (value != null) {
                          if (value['status_code'] == 200) {
                            setState(() {
  ...
  void getData() {
    ...
    widget.Istryers ? API(context).get(
        'part/category/attributes/${widget.Category_id}').then((value) {
      print(value);
      if (value != null) {
        setState(() {
          if (value['data'] != null) {
...
setState(() {
  ...
  ExpansionCatgory(Categories_item categories_item,
      ...
      children: [
        categories_item.categories == null
            ...
            itemBuilder:
                (BuildContext context,
                int i) {
              return categories_item.categories[i].level == 0 ? Row(
                children: [
                  Checkbox(
                      value: categories_item.categories[i]
                          .Check,
                      activeColor:
                      Colors.orange,
                      onChanged: (value) {
                        setState(() {
                          categories_item.categories[i].Check = value;
                        });
                        value
                            ? partSelect.add(categories_item.categories[i].id)
                            : partSelect.remove(
                            categories_item.categories[i].id);
                      }),
                  Text(
                    "${themeColor.getlocal() == 'ar' ? categories_item
                        .categories[i].name ??
                        categories_item.categories[i].name : categories_item
                        .categories[i].nameEn ??
                        categories_item.categories[i].name}",
                    softWrap: true,
                  ),
                ],
              ) : ExpansionCatgory(categories_item.categories[i], themeColor);
            })
      ],
    );
  }
